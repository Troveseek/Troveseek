...
import { 
  ...
  X, Globe, DollarSign, Percent, CheckCircle2, CreditCard, Trash2
} from 'lucide-react';
...
export default function EditCountryPage() {
      ...
      try {
        ...
        if (c) {
          setForm({
            ...
            taxRate: c.taxRate || 0,
            methods: c.methodsList || parsePaymentMethods(c.methods),
        ...
        setError('Failed to load country details from server.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || 'Failed to update country');

      toast.success(`Country "${form.name}" updated successfully`);
      router.push('/admin/countries');
      router.refresh();
    } catch (err: any) {
      setError(err.message || 'Failed to update country. Please try again.');
  ...
  if (loading) {
    return (
      <div style={{ padding: '64px 24px', textAlign: 'center' }}>
        ...
        <div style={{ fontSize: '16px', fontWeight: 600, color: 'var(--clr-text)' }}>Loading Country Settings...</div>
      </div>
    );
  }
        ...
        <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <h1 style={{ fontSize: '28px', fontFamily: 'var(--font-display)', fontWeight: 700, margin: 0, color: 'var(--clr-text)' }}>
              Edit Country: {form.name}
            </h1>
          </div>
          <p style={{ color: 'var(--clr-text-muted)', fontSize: '14px', marginTop: '4px', marginBottom: 0 }}>
            Update regional tax policies, currency codes, and active checkout gateways.
          </p>
        </div>
            ...
            <CardBody style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
              ...
              <div>
                ...
                <input 
                  ...
                  placeholder="e.g. Algeria, United States" 
                />
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                <div>
                  ...
                  <p style={{ fontSize: '11px', color: 'var(--clr-text-muted)', marginTop: '4px' }}>
                    ISO-3166-1 alpha-2 (e.g. dz, us, gb)
                  </p>
                </div>
