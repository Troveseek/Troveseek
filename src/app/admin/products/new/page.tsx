...
import {
  ...
  Link as LinkIcon, Image as ImageIcon, Search, TrendingUp, Eye, ShoppingCart
} from 'lucide-react';
      ...
      <div style={{ flex: '3', minWidth: '300px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center' }}>
            ...
            <input type="text" placeholder="nextjs-saas-starter" value={slug} onChange={e => setSlug(e.target.value)} style={{ flex: 1, padding: '10px 12px', background: 'var(--clr-surface)', border: '1px solid var(--clr-border)', borderRadius: '0 8px 8px 0', color: 'var(--clr-text)', fontSize: '14px', outline: 'none' }} />
          </div>
        </div>

        <div>
          <label style={{ display: 'block', fontSize: '14px', fontWeight: 500, marginBottom: '8px', color: 'var(--clr-text)' }}>Short Description (EN)</label>
          <textarea placeholder="Brief summary of the product (shown in listings)..." rows={3} value={description} onChange={e => setDescription(e.target.value)} style={{ width: '100%', padding: '12px', background: 'var(--clr-surface)', border: '1px solid var(--clr-border)', borderRadius: '8px', color: 'var(--clr-text)', fontSize: '14px', outline: 'none', resize: 'vertical', boxSizing: 'border-box' }} />
        </div>
        <div>
          <label style={{ display: 'block', fontSize: '14px', fontWeight: 500, marginBottom: '8px', color: 'var(--clr-text)' }}>Short Description (AR)</label>
          <textarea placeholder="وصف قصير..." dir="rtl" rows={3} value={descriptionAr} onChange={e => setDescriptionAr(e.target.value)} style={{ width: '100%', padding: '12px', background: 'var(--clr-surface)', border: '1px solid var(--clr-border)', borderRadius: '8px', color: 'var(--clr-text)', fontSize: '14px', outline: 'none', resize: 'vertical', boxSizing: 'border-box' }} />
        </div>
        ...
        <div>
          ...
          <div style={{ border: '1px solid var(--clr-border)', borderRadius: '8px', overflow: 'hidden' }}>
            <textarea
              ...
              style={{ width: '100%', padding: '16px', background: 'var(--clr-surface)', border: 'none', color: 'var(--clr-text)', fontSize: '14px', outline: 'none', resize: 'vertical', boxSizing: 'border-box', fontFamily: 'inherit', lineHeight: 1.7 }}
            />
          </div>
        </div>
        <div>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginTop: '8px' }}>
                  {tags.map(tag => (
                    <span key={tag} style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', padding: '3px 8px', background: 'var(--clr-primary)', color: 'white', fontSize: '12px', borderRadius: '999px' }}>
                      {tag}
                      <button type="button" onClick={() => removeTag(tag)} style={{ background: 'none', border: 'none', color: 'white', cursor: 'pointer', padding: 0, lineHeight: 1 }}>
                        <X size={10} />
                      </button>
                    </span>
                  ))}
  ...
  const pricingTab = (
    <div style={{ maxWidth: '680px', marginTop: '24px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
      <div style={{ display: 'flex', gap: '16px' }}>
        <div style={{ flex: 1 }}>
          <Input label="Regular Price (USD) *" type="number" placeholder="49.99" value={price} onChange={e => setPrice(e.target.value)} />
        </div>
        <div style={{ flex: 1 }}>
          <Input label="Sale Price (Optional)" type="number" placeholder="39.99" value={salePrice} onChange={e => setSalePrice(e.target.value)} />
        </div>
        <div style={{ flex: 1 }}>
          <Input label="Stock / Quantity" type="number" placeholder="0 = Unlimited" value={stock} onChange={e => setStock(e.target.value)} />
        </div>
      </div>
    ...
    <div style={{ marginTop: '24px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
        ...
        {[
          ...
          <div
            ...
            style={{
              ...
              padding: '20px',
              borderRadius: '12px',
              border: productType === type ? '2px solid var(--clr-primary)' : '1px solid var(--clr-border)',
              background: productType === type ? 'rgba(124, 111, 255, 0.05)' : 'var(--clr-surface-elevated)',
              cursor: 'pointer',
              transition: 'all 0.2s',
            }}
          >
      ...
      {productType === 'download' && (
        <div>
          ...
          <div
            ...
            style={{ border: '2px dashed var(--clr-border)', borderRadius: '12px', padding: '48px', textAlign: 'center', background: 'var(--clr-surface-elevated)', cursor: isSubmitting ? 'not-allowed' : 'pointer', transition: 'border-color 0.2s', opacity: isSubmitting ? 0.7 : 1 }}
          >
            <UploadCloud size={40} style={{ margin: '0 auto 16px auto', color: fileUrl ? 'var(--clr-primary)' : 'var(--clr-text-muted)' }} />
            <h4 style={{ fontSize: '16px', margin: '0 0 8px 0', color: fileUrl ? 'var(--clr-primary)' : 'var(--clr-text)' }}>
              {fileUrl ? `✓ ${fileUrl}` : 'Upload product file'}
            </h4>
      ...
      {productType === 'key' && (
        <div>
          ...
          <textarea placeholder={'XXXX-XXXX-XXXX-XXXX\nYYYY-YYYY-YYYY-YYYY'} rows={10} style={{ width: '100%', padding: '16px', background: 'var(--clr-surface-elevated)', border: '1px solid var(--clr-border)', borderRadius: '8px', color: 'var(--clr-text)', fontFamily: 'var(--font-mono, monospace)', fontSize: '14px', outline: 'none', resize: 'vertical', boxSizing: 'border-box' }} />
          <p style={{ fontSize: '13px', color: 'var(--clr-text-muted)', marginTop: '8px' }}>Keys are automatically assigned to customers upon purchase.</p>
        </div>
      )}
    ...
    <div style={{ marginTop: '24px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={{ border: '1px solid var(--clr-border)', borderRadius: '16px', padding: '32px', background: 'var(--clr-surface)' }}>
        ...
        <div onClick={() => imageInputRef.current?.click()} onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); Array.from(e.dataTransfer.files).forEach(file => { const r = new FileReader(); r.onload = ev => setImages(p => [...p, ev.target?.result as string]); r.readAsDataURL(file); }); }} style={{ border: '2px dashed var(--clr-border)', borderRadius: '12px', padding: '40px', textAlign: 'center', background: 'var(--clr-surface-elevated)', cursor: 'pointer' }}>
          <input type="file" multiple ref={imageInputRef} onChange={handleImageUpload} style={{ display: 'none' }} accept="image/*" />
          <UploadCloud size={32} color="var(--clr-primary)" style={{ margin: '0 auto 16px' }} />
          <h4 style={{ margin: '0 0 8px 0', fontSize: '16px' }}>Upload Product Images</h4>
            ...
            {images.map((src, i) => (
              <div key={i} style={{ position: 'relative', borderRadius: '8px', overflow: 'hidden', aspectRatio: '1', border: '1px solid var(--clr-border)' }}>
                ...
                <button
                  ...
                  style={{ position: 'absolute', top: '4px', right: '4px', background: 'rgba(0,0,0,0.7)', border: 'none', borderRadius: '50%', width: '24px', height: '24px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: 'white' }}
                >
                  <X size={12} />
                </button>
                {i === 0 && (
                  <span style={{ position: 'absolute', bottom: '4px', left: '4px', background: 'var(--clr-primary)', color: 'white', fontSize: '10px', padding: '2px 6px', borderRadius: '4px' }}>Main</span>
                )}
              </div>
            ))}
          </div>
    ...
    <div style={{ maxWidth: '680px', marginTop: '24px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        <div>
          ...
          <input type="text" placeholder={`${name || 'Product Name'} | TroveSeek`} value={metaTitle} onChange={e => setMetaTitle(e.target.value)} maxLength={70} style={{ width: '100%', padding: '10px 12px', background: 'var(--clr-surface)', border: '1px solid var(--clr-border)', borderRadius: '8px', color: 'var(--clr-text)', fontSize: '14px', outline: 'none', boxSizing: 'border-box' }} />
        </div>
        <div>
          <label style={{ display: 'block', fontSize: '14px', fontWeight: 500, marginBottom: '8px', color: 'var(--clr-text)' }}>Meta Title (AR)</label>
          <input type="text" placeholder={`${nameAr || 'الاسم بالعربية'} | TroveSeek`} value={metaTitleAr} onChange={e => setMetaTitleAr(e.target.value)} maxLength={70} dir="rtl" style={{ width: '100%', padding: '10px 12px', background: 'var(--clr-surface)', border: '1px solid var(--clr-border)', borderRadius: '8px', color: 'var(--clr-text)', fontSize: '14px', outline: 'none', boxSizing: 'border-box' }} />
        </div>

        <div>
          <label style={{ display: 'block', fontSize: '14px', fontWeight: 500, marginBottom: '8px', color: 'var(--clr-text)' }}>Meta Description (EN)</label>
          <textarea placeholder={description || 'Write a compelling description for search engines...'} value={metaDescription} onChange={e => setMetaDescription(e.target.value)} maxLength={170} rows={3} style={{ width: '100%', padding: '12px', background: 'var(--clr-surface)', border: '1px solid var(--clr-border)', borderRadius: '8px', color: 'var(--clr-text)', fontSize: '14px', outline: 'none', resize: 'vertical', boxSizing: 'border-box' }} />
        </div>
        <div>
          <label style={{ display: 'block', fontSize: '14px', fontWeight: 500, marginBottom: '8px', color: 'var(--clr-text)' }}>Meta Description (AR)</label>
          <textarea placeholder={descriptionAr || 'وصف لمحركات البحث...'} dir="rtl" value={metaDescriptionAr} onChange={e => setMetaDescriptionAr(e.target.value)} maxLength={170} rows={3} style={{ width: '100%', padding: '12px', background: 'var(--clr-surface)', border: '1px solid var(--clr-border)', borderRadius: '8px', color: 'var(--clr-text)', fontSize: '14px', outline: 'none', resize: 'vertical', boxSizing: 'border-box' }} />
        </div>
      </div>
      ...
      <div style={{ padding: '20px', background: 'var(--clr-surface-elevated)', borderRadius: '12px', border: '1px solid var(--clr-border)' }}>
        ...
        <div style={{ fontFamily: 'Arial, sans-serif' }}>
          <div style={{ fontSize: '13px', color: 'var(--clr-text-muted)', marginBottom: '4px' }}>troveseek.com › shop › {slug || 'product-slug'}</div>
          <div style={{ fontSize: '18px', color: '#1a0dab', marginBottom: '4px', overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>
            {metaTitle || name || 'Product Name | TroveSeek'}
          </div>
          <div style={{ fontSize: '14px', color: '#4d5156', lineHeight: 1.5 }}>
            {(metaDescription || description || 'Your product description will appear here in Google search results.').slice(0, 160)}
          </div>
        </div>
      </div>
    </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          ...
          {features.map((feature, index) => (
            <div key={index} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <input type="text" placeholder="EN Feature (e.g. Water resistant)" value={feature} onChange={e => { const newF = [...features]; newF[index] = e.target.value; setFeatures(newF); }} style={{ flex: 1, padding: '10px', background: 'var(--clr-surface)', border: '1px solid var(--clr-border)', borderRadius: '6px', color: 'var(--clr-text)', outline: 'none' }} />
              <input type="text" placeholder="AR Feature (مقاوم للماء)" dir="rtl" value={featuresAr[index] || ''} onChange={e => { const newF = [...featuresAr]; newF[index] = e.target.value; setFeaturesAr(newF); }} style={{ flex: 1, padding: '10px', background: 'var(--clr-surface)', border: '1px solid var(--clr-border)', borderRadius: '6px', color: 'var(--clr-text)', outline: 'none' }} />
            ...
            <div key={index} style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', padding: '12px', background: 'var(--clr-surface)', border: '1px solid var(--clr-border)', borderRadius: '8px' }}>
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <div style={{ display: 'flex', gap: '8px' }}>
                  <input type="text" placeholder="EN Name (e.g. Weight)" value={spec.name} onChange={e => { const newS = [...specifications]; newS[index].name = e.target.value; setSpecifications(newS); }} style={{ flex: 1, padding: '10px', background: 'var(--clr-surface-elevated)', border: '1px solid var(--clr-border)', borderRadius: '6px', color: 'var(--clr-text)', outline: 'none' }} />
                  <input type="text" placeholder="EN Value (e.g. 1.5kg)" value={spec.value} onChange={e => { const newS = [...specifications]; newS[index].value = e.target.value; setSpecifications(newS); }} style={{ flex: 1, padding: '10px', background: 'var(--clr-surface-elevated)', border: '1px solid var(--clr-border)', borderRadius: '6px', color: 'var(--clr-text)', outline: 'none' }} />
                </div>
                <div style={{ display: 'flex', gap: '8px' }}>
                  <input type="text" placeholder="AR Name (الوزن)" dir="rtl" value={specificationsAr[index]?.name || ''} onChange={e => { const newS = [...specificationsAr]; if(!newS[index]) newS[index] = {name:'', value:''}; newS[index].name = e.target.value; setSpecificationsAr(newS); }} style={{ flex: 1, padding: '10px', background: 'var(--clr-surface-elevated)', border: '1px solid var(--clr-border)', borderRadius: '6px', color: 'var(--clr-text)', outline: 'none' }} />
                  <input type="text" placeholder="AR Value (1.5 كجم)" dir="rtl" value={specificationsAr[index]?.value || ''} onChange={e => { const newS = [...specificationsAr]; if(!newS[index]) newS[index] = {name:'', value:''}; newS[index].value = e.target.value; setSpecificationsAr(newS); }} style={{ flex: 1, padding: '10px', background: 'var(--clr-surface-elevated)', border: '1px solid var(--clr-border)', borderRadius: '6px', color: 'var(--clr-text)', outline: 'none' }} />
                </div>
              </div>
      ...
      <div style={{ background: 'var(--clr-surface-elevated)', padding: '24px', borderRadius: '12px', border: '1px solid var(--clr-border)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          ...
          {requirements.map((req, index) => (
            <div key={index} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <input type="text" placeholder="EN Req (e.g. Windows 10)" value={req} onChange={e => { const newR = [...requirements]; newR[index] = e.target.value; setRequirements(newR); }} style={{ flex: 1, padding: '10px', background: 'var(--clr-surface)', border: '1px solid var(--clr-border)', borderRadius: '6px', color: 'var(--clr-text)', outline: 'none' }} />
              <input type="text" placeholder="AR Req (ويندوز 10)" dir="rtl" value={requirementsAr[index] || ''} onChange={e => { const newR = [...requirementsAr]; newR[index] = e.target.value; setRequirementsAr(newR); }} style={{ flex: 1, padding: '10px', background: 'var(--clr-surface)', border: '1px solid var(--clr-border)', borderRadius: '6px', color: 'var(--clr-text)', outline: 'none' }} />
  ...
  const analyticsTab = (
    <div style={{ marginTop: '24px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '16px' }}>
        {[
          { label: 'Total Views', value: '—', icon: <Eye size={20} />, color: 'var(--clr-primary)' },
          { label: 'Total Sales', value: '—', icon: <ShoppingCart size={20} />, color: '#00e5b0' },
          { label: 'Revenue', value: '—', icon: <TrendingUp size={20} />, color: '#f59e0b' },
          { label: 'Conversion Rate', value: '—', icon: <BarChart2 size={20} />, color: '#ec4899' },
        ].map(stat => (
          <div key={stat.label} style={{ padding: '20px', background: 'var(--clr-surface-elevated)', borderRadius: '12px', border: '1px solid var(--clr-border)' }}>
            ...
            <div style={{ fontSize: '24px', fontWeight: 700, fontFamily: 'var(--font-display)' }}>{stat.value}</div>
            <div style={{ fontSize: '13px', color: 'var(--clr-text-muted)', marginTop: '4px' }}>{stat.label}</div>
          </div>
        ))}
      </div>
      <div style={{ padding: '24px', background: 'var(--clr-surface-elevated)', border: '1px dashed var(--clr-border)', borderRadius: '12px', textAlign: 'center' }}>
        <BarChart2 size={40} style={{ margin: '0 auto 12px', color: 'var(--clr-text-muted)', opacity: 0.4 }} />
        <p style={{ color: 'var(--clr-text-muted)', fontSize: '14px' }}>Analytics will be tracked automatically after you publish this product.</p>
      </div>
    </div>
  );
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          ...
          <h1 style={{ fontSize: '28px', fontFamily: 'var(--font-display)', margin: 0, color: 'var(--clr-text)' }}>Add New Product</h1>
        </div>
