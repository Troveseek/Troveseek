...
export default function EditSaaSPage() {
  ...
  const [plansAr, setPlansAr] = useState<any[]>([]);
  const [whyChooseUs, setWhyChooseUs] = useState<any[]>([]);
  const [whyChooseUsAr, setWhyChooseUsAr] = useState<any[]>([]);
    ...
    fetch(`/api/saas/${id}`)
      ...
      .then(res => {
        if (res.data) {
          ...
          setTaglineAr(s.taglineAr || '');
          setDescription(s.description || '');
          setDescriptionAr(s.descriptionAr || '');
          setDemoUrl(s.demoUrl || '');
          setLogo(s.logo || '');
          setStatus(s.status || 'ACTIVE');
          setPlatform(s.platform || 'Web App');
          setHasFreeTrial(!!s.hasFreeTrial);
          setDocumentationUrl(s.documentationUrl || '');
          setCommunityUrl(s.communityUrl || '');
          setGithubUrl(s.githubUrl || '');
          setCategoryId(s.categoryId || '');
          setMetaTitle(s.metaTitle || '');
          setMetaTitleAr(s.metaTitleAr || '');
          setMetaDescription(s.metaDescription || '');
          setMetaDescriptionAr(s.metaDescriptionAr || '');
          ...
          setPlans(safeParse(s.plans, []));
          setPlansAr(safeParse(s.plansAr, []));
          setFeatures(safeParse(s.features, []));
          setFeaturesAr(safeParse(s.featuresAr, []));
          setImages(safeParse(s.images, []));
          setWhyChooseUs(safeParse(s.whyChooseUs, []));
          setWhyChooseUsAr(safeParse(s.whyChooseUsAr, []));
          setFaqs(safeParse(s.faqs, []));
          setFaqsAr(safeParse(s.faqsAr, []));
        }
      })
    ...
    try {
      const res = await fetch(`/api/saas/${id}`, {
        ...
        body: JSON.stringify({
          ...
          monthlyPrice: plans.length > 0 ? (plans[0].monthlyPrice ?? plans[0].price ?? 0) : 0,
          yearlyPrice: plans.length > 0 ? (plans[0].yearlyPrice ?? (plans[0].price ? plans[0].price * 10 : 0)) : 0,
      ...
      if (!res.ok) {
        const data = await res.json();
        throw new Error(data.error || 'Failed to update SaaS');
      }
  ...
  const handleDelete = async () => {
    if (!window.confirm('Are you sure you want to delete this SaaS product?')) return;
    try {
  ...
  return (
    <form className={styles.formPage} onSubmit={handleSubmit}>
      <div className={styles.pageHeader}>
        <div>
          <h1 className={styles.title}>Edit SaaS Product</h1>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px', gap: '16px', flexWrap: 'wrap' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <div style={{ display: 'flex', flexDirection: 'column' }}>
                        ...
                        <input type="number" className={styles.formInput} value={plan.monthlyPrice !== undefined ? plan.monthlyPrice : plan.price} onChange={e => handleUpdatePlan(index, 'monthlyPrice', parseFloat(e.target.value))} style={{ width: '90px' }} required min="0" />
                      </div>
                      <div style={{ display: 'flex', flexDirection: 'column' }}>
                        <span style={{ fontSize: '12px', color: 'var(--clr-text-muted)' }}>Yearly ($)</span>
                        <input type="number" className={styles.formInput} value={plan.yearlyPrice !== undefined ? plan.yearlyPrice : (plan.price ? plan.price * 10 : 0)} onChange={e => handleUpdatePlan(index, 'yearlyPrice', parseFloat(e.target.value))} style={{ width: '90px' }} required min="0" />
                      </div>
        ...
        <div className={styles.sideCol}>
          ...
          <div className={styles.card} style={{ border: '1px solid #fee2e2' }}>
            ...
            <p style={{ fontSize: '13px', color: 'var(--clr-text-muted)', marginBottom: '16px' }}>
              Permanently delete this SaaS product and all of its data.
            </p>
            <Button type="button" onClick={handleDelete} style={{ background: '#fee2e2', color: '#ef4444', width: '100%' }}>
              Delete Product
            </Button>
          </div>
        </div>
