...
export default function EditGalleryImagePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    fetch('/api/gallery')
      .then(res => res.json())
      .then(data => {
        const image = (data.data || []).find((i: any) => i.id === id);
        if (image) {
          setTitle(image.title || '');
          setTitleAr(image.titleAr || '');
          setImageUrl(image.imageUrl || '');
          setCaption(image.caption || '');
          setCaptionAr(image.captionAr || '');
  ...
  return (
    <form className={styles.formPage} onSubmit={handleSave}>
      <div className={styles.pageHeader}>
        <div>
          <h1 className={styles.title}>Edit Gallery Image</h1>
          <p className={styles.subtitle}>Update image details in your gallery.</p>
        </div>
