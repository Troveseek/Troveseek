...
export default function EditCategoryPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const cat = json.data?.find((c: any) => c.id === id);
        if (cat) {
          setName(cat.name);
  ...
  if (isLoading) {
    return <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--clr-text-muted)', padding: '40px' }}><Loader size={20} className="spin" /> Loading category...</div>;
  }
    ...
    <form className={styles.formPage} onSubmit={handleSubmit}>
      ...
      {success && (
        <div style={{ padding: '12px 16px', background: '#dcfce7', color: '#22c55e', borderRadius: '8px', fontSize: '14px', marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '8px' }}>
          <CheckCircle2 size={16} /> Category updated successfully! Redirecting...
        </div>
      )}
