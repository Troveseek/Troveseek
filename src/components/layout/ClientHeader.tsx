...
import { Globe, ShoppingCart, Heart, Menu } from 'lucide-react';
    ...
    <div style={{ position: 'sticky', top: 0, zIndex: 100, display: 'flex', flexDirection: 'column' }}>
      <header className={`${styles.header} ${scrolled ? styles.scrolled : ''}`}>
        ...
        <div className={styles.actions}>
          <ClientSearch />
          <ThemeToggle />
          <button className={styles.iconBtn} aria-label={isAr ? 'تغيير اللغة' : 'Change language'} onClick={handleLanguageSwitch} style={{ display: 'flex', alignItems: 'center', gap: '6px', width: 'auto', padding: '0 12px' }}>
            <Globe size={20} />
          ...
          <Link href="/favorites" className={styles.iconBtn} aria-label={isAr ? 'المفضلة' : 'Favorites'} style={{ position: 'relative' }}>
            <Heart size={20} />
