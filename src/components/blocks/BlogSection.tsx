      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '48px', flexWrap: 'wrap', gap: '16px' }}>
          <div>
            ...
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '32px', fontWeight: 700 }}>{isAr ? 'أحدث المقالات' : 'Latest from the Blog'}</h2>
          </div>
          <Link href="/blog" style={{ color: 'var(--clr-primary)', fontWeight: 600, textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '4px' }}>
            ...
            <Link key={post.id} href={`/blog/${post.slug}`} style={{ textDecoration: 'none' }}>
              <div style={{
                ...
                {post.coverImage ? (
                  ...
                  <div style={{ height: '200px', background: 'linear-gradient(135deg, var(--clr-surface-2), var(--clr-surface-3))' }} />
                )}
                
                <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', flex: 1 }}>
                  {post.category && (
                    <span style={{ display: 'inline-block', background: 'var(--clr-primary-dim)', color: 'var(--clr-primary)', fontSize: '11px', fontWeight: 600, textTransform: 'uppercase', padding: '4px 10px', borderRadius: '999px', marginBottom: '16px', alignSelf: 'flex-start' }}>
                      {post.category}
                    </span>
                  )}
                  <h3 style={{ fontSize: '20px', fontWeight: 600, marginBottom: '12px', color: 'var(--clr-text)' }}>{isAr ? (post.titleAr || post.title) : post.title}</h3>
